...
class PaymentRepositoryImpl {
    ...
    async create(payment) {
        ...
        try {
            ...
            if (!payment.id || payment.id.trim() === '') {
                payment.id = (0, uuid_1.v4)(); // Generate a unique ID
            }
    ...
    async updateStatus(payment_intent_id, status) {
    ...
    async updateTransferStatus(id, type, status) {
        let query;
    ...
    async findBySessionId(sessionId) {
        ...
        return new payment_1.PaymentEntity('', row.session_id, row.user_id, row.instructor_id, row.course_id, row.amount_in_inr, row.amount_in_usd, row.admin_amount, row.instructor_amount, row.currency, row.status, row.created_at, row.updated_at, row.admin_account_id, row.instructor_account_id, row.admin_payout_status, row.instructor_payout_status);
    }
    async findPaymentIntentId(userId, courseId) {
        ...
        return new payment_1.PaymentEntity('', row.session_id, row.stripe_payment_intent_id, row.user_id, row.instructor_id, row.course_id, row.amount_in_inr, row.amount_in_usd, row.admin_amount, row.instructor_amount, row.currency, row.status, row.created_at, row.updated_at, row.admin_account_id, row.instructor_account_id, row.admin_payout_status, row.instructor_payout_status);
    }
    ...
    `;
        ...
        try {
            ...
            const transactions = transactionsResult.rows.map(row => new payment_1.PaymentEntity('', row.session_id, row.stripe_payment_intent_id, row.user_id, row.instructor_id, row.course_id, row.amount_in_inr, row.amount_in_usd, row.admin_amount, row.instructor_amount, row.currency, row.status, row.created_at, row.updated_at, row.admin_account_id, row.instructor_account_id, row.admin_payout_status, row.instructor_payout_status));
            // Fetch total count
    ...
    async findByInstructorId(instructorId, limit, offset) {
        const query = `
      SELECT
        id,
        user_id,
    ...
    `;
        ...
        try {
            ...
            return result.rows.map(row => new payment_1.PaymentEntity('', row.session_id, row.user_id, row.instructor_id, row.course_id, row.amount_in_inr, row.amount_in_usd, row.admin_amount, row.instructor_amount, row.currency, row.status, row.created_at, row.updated_at, row.admin_account_id, row.instructor_account_id, row.admin_payout_status, row.instructor_payout_status));
        }
        catch (error) {
            console.error('Error retrieving transactions:', error);
            throw new Error('Failed to retrieve transactions');
    ...
    async findByStudentId(studentId, limit, offset) {
    ...
    `;
        ...
        try {
            const result = await this.pool.query(query, values);
            return result.rows.map(row => new payment_1.PaymentEntity('', row.session_id, row.stripe_payment_intent_id, row.user_id, row.instructor_id, row.course_id, row.amount_in_inr, row.amount_in_usd, row.admin_amount, row.instructor_amount, row.currency, row.status, row.created_at, row.updated_at, row.admin_account_id, row.instructor_account_id, row.admin_payout_status, row.instructor_payout_status));
        }
        catch (error) {
            console.error('Error retrieving transactions:', error);
            throw new Error('Failed to retrieve transactions');
    ...
    async getTodayRevenueForInstructor(instructorId) {
    ...
    async getTotalEarningsForInstructor(instructorId) {
        const query = `
    ...
    async getTotalEarningsForAdmin() {
        const query = `
